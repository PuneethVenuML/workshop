...
# PV: for each value in array, we get rgb values
...
print('xtrain.shape 1',xtrain.shape)

# ## Convert to 1 + 4D space (1st argument represents number of rows in the dataset)
xtrain = xtrain.reshape(x_train.shape[0], 16, 16, 16, 3)
xtest = xtest.reshape(x_test.shape[0], 16, 16, 16, 3)
print('xtrain.shape 2',xtrain.shape)
...
# pd.crosstab(pd.Series(ytest))
import pandas as pd
df = pd.crosstab(pd.Series(ytest), pd.Series(pred))
df
